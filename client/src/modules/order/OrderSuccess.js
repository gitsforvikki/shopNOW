...
let OrderSuccess = () => {
    ...
    let { orderPlcaced , loading } = orderInfo;


    // calc Grand Total
    let calculateGrandTotal = () => {
        let total = 0;
        if(orderPlcaced.items?.length > 0){
            for(let item of orderPlcaced.items){
                total += Number(item.qty) * Number(item.price);
            }
        }
        return (total + Number(orderPlcaced.tax));
    };
                ...
                loading ? <Spinner/> :
                    <React.Fragment>
                        <section className="mt-3 p-3">
                            {
                                Object.keys(orderPlcaced).length > 0 ? <React.Fragment>
                                    <div className="container">
                                        <div className="row">
                                                    ...
                                                    <div className="card-body">
                                                        <ul className="list-group">
                                                            <li className="list-group-item">
                                                                Order ID : {orderPlcaced._id}
                                                            </li>
                                                            <li className="list-group-item">
                                                                NAME : {orderPlcaced.name}
                                                            </li>
                                                            <li className="list-group-item">
                                                                Email : {orderPlcaced.email}
                                                            </li>
                                                            <li className="list-group-item">
                                                                Mobile : {orderPlcaced.mobile}
                                                            </li>
                                                            <li className="list-group-item">
                                                                DATE : {new Date(orderPlcaced.createdAt).toLocaleDateString()}
                                                            </li>
                                                        </ul>
                                                        <table className="table table-hover text-left table-striped table-light">
                                                            ...
                                                            <tbody>
                                                            {
                                                                orderPlcaced.items?.length > 0 ? <React.Fragment>
                                                                    {
                                                                        orderPlcaced.items?.map((item, index) => {
                                                                            return (
                                                                                <tr key={item._id}>
                                                                                    <td>{index+1}</td>
                                                                                    <td>{item.name}</td>
                                                            ...
                                                            {
                                                                Object.keys(orderPlcaced).length > 0 &&
                                                                <tr>
                                                                    <td colSpan="4"/>
                                                                    <td>Tax : &#8377;
                                                                        <span className="text-danger font-weight-bold">{Number(orderPlcaced.tax)?.toFixed(2)}</span>
                                                                    </td>
                                                                </tr>
                                                            }
                                                            <tr>
